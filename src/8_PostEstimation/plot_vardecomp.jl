...
function plot_vardecomp(
    ...
    df = vcat(
        [
            ...
            ) for i = 1:length(model_names), h in select_vd_horizons,
            k = 1:length(SHOCKs), j = 1:length(select_variables)
        ]...,
    )
    ...
    df_bc = vcat(
        [
            DataFrame(;
                ...
                Shock = SHOCKs[k],
                Variable = select_variables[j],
            ) for i = 1:length(model_names), k = 1:length(SHOCKs),
            j = 1:length(select_variables)
        ]...,
    )
    ...
    for j in select_variables
        ...
        if savepdf
            p |>
            save(string("8_PostEstimation/Figures/CVD/CVD_at_bcf_of_", j, suffix, ".pdf"))
        end
        display(p)
    end
