...
function prepare_linearization(KSS, VmSS, VkSS, distrSS, n_par, m_par)
    ...
    taxrev[end] =
        n_par.grid_y[end] .* profitsSS -
        m_par.τ_lev .* (n_par.grid_y[end] .* profitsSS) .^ (1.0 - m_par.τ_prog)
    ...
    incgross, incnet, eff_int = incomes(
        n_par,
        m_par,
        1.0 ./ m_par.μw,
        1.0,
    ...
    VmSS,
    VkSS,
    distrSS = Ksupply(m_par.RB, rSS, n_par, m_par, VmSS, VkSS, distrSS, incnet, eff_int)
    ...
    # ------------------------------------------------------------------------------
    SELECT = [
        ((i + j + k) <= n_par.reduc_copula) &
        (!((i == 1) & (j == 1)) & !((k == 1) & (j == 1)) & !((k == 1) & (i == 1))) for
        i = 1:(n_par.nm_copula), j = 1:(n_par.nk_copula), k = 1:(n_par.ny_copula)
    ]
    ...
    @set! n_par.ntotal =
        ...
        (n_par.ny + n_par.nm + n_par.nk - 3 + n_par.naggr)
    @set! n_par.nstates =
        n_par.ny + n_par.nk + n_par.nm - 3 +
        n_par.naggrstates +
        length(compressionIndexes[3]) # add to no. of states the coefficients that perturb the copula
